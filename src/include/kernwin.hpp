...
#define WOPN_SZHINT            0x00000200u ///< when floating or in a splitter (i.e., not tabbed),
...
struct hexplace_gen_t
{
  ...
  virtual bool is_edited_byte(ea_t ea, uint64 *out_value=NULL) const = 0;
...
typedef lecvt_code_t idaapi lochist_entry_cvt_t(
        lochist_entry_t *dst,
...
struct lines_rendering_input_t
{
  ...
  const synced_group_t *sync_group;     ///< the 'synced' group 'widget' (see ui_get_lines_rendering_info) belongs to, or NULL
...
#define IWID_TRACE         (1ULL << BWN_TRACE        ) ///< trace view        (16)
...
#define CHCOL_FNAME     0x00050000  ///< function name
...
public:
  ...
  enum
  {
    ...
    EMPTY_CHOOSER  = -2,  ///< the chooser has no data and cannot be
                          ///< displayed
...
struct chooser_t : public chooser_base_t
{
  ...
  /// \return idx      index of the selected item (cursor)
  ///         changed  what is changed
  inline virtual cbret_t idaapi enter(size_t n) newapi
  {
  ...
  /// \return the dirtree_t, or NULL
  virtual dirtree_t *idaapi get_dirtree() newapi { return NULL; }
...
//@}
  ...
  void reset()
  {
    ...
    //
    cur_flags = ACF_HAS_FIELD_DIRTREE_SELECTION;
    cur_ea = BADADDR;
    cur_value = BADADDR;
...
struct action_desc_t
{
  ...
  const void *owner;         ///< either the plugin_t, or plugmod_t responsible for
                             ///< registering the action. Can be NULL
...
/// \return handle to registered timer (use this handle to unregister it), or NULL
...
/// If 'menupath' is non-NULL, it provides information about where
...
{
  action_desc_t desc = ACTION_DESC_LITERAL_OWNER(name, label, handler, owner, shortcut, NULL, -1, action_desc_t_flags);
...
/// \param dest_ctrl where to dock: if NULL or invalid then
///                  use the active docker if there is not
///                  create a new tab relative to current active tab

inline void display_widget(TWidget *widget, uint32 options, const char *dest_ctrl=NULL)
{
...
/// this function will return NULL even if it is currently shown.
...
/// \param the name of the window (NULL means the main IDA window)
/// \return the low-level window ID

inline void *get_window_id(const char *name=NULL)
{
...
struct addon_info_t
{
  ...
  const char *freeform;     //< any string, e.g. "Copyright (c) 2007-2020 Hex-Rays"
...
enum cb_id
{
  ...
  CB_DESTROYING = -5, // when the actual widget tree is being destroyed
};
