...
#define         IDD_INTERFACE_VERSION   26
...
typedef qvector<call_stack_info_t> call_stack_t;
...
#define DEBUGGER_PORT_NUMBER 23946
  ...
  enum event_t
  {
    ...
    /// \param errbuf (::qstring *) may be NULL
    ...
    /// it has suspended the debugger process and refreshed the database.
    /// The debugger module may add information to the database if it wants.
    ///
    /// The reason for introducing this event is that when an event line
    ...
    /// in the get_debug_event() event.
    /// Only when the kernel has adjusted the database we can do it.
    /// Example: for imported PE DLLs we will add the exported event
    /// names to the database.
    ///
    /// This event is generated in the main thread.
    /// \param dlls_added (bool)
    /// \param thr_names  (::thread_name_vec_t *) (for the kernel only, must be NULL)
    /// \return ::DRC_NONE, ::DRC_OK
    ev_suspended,
    ...
    /// \param answer      (::ea_t *) pointer to the answer. can't be NULL.
    /// \param tid         (::thid_t) thread id
    ...
    ///                 if regs == NULL, then perform global mapping,
    ...
    /// This event is generated when the process is suspended and should fill
    /// the 'trace' object with the information about the current call stack.
    /// If this event returns DRC_NONE, IDA will use the standard
  ...
  inline bool init_debugger(const char *hostname, int portnum, const char *password, qstring *errbuf=NULL);
  inline bool term_debugger(void);
  inline drc_t get_processes(procinfo_vec_t *procs, qstring *errbuf=NULL);
  inline drc_t start_process(const char *path,
                             ...
                             qstring *errbuf=NULL);
  inline drc_t attach_process(pid_t pid, int event_id, uint32 dbg_proc_flags, qstring *errbuf=NULL);
  inline drc_t detach_process(qstring *errbuf=NULL);
  inline bool get_debapp_attrs(debapp_attrs_t *out_pattrs);
  inline void rebase_if_required_to(ea_t new_base);
  inline drc_t request_pause(qstring *errbuf=NULL);
  inline drc_t exit_process(qstring *errbuf=NULL);
  inline gdecode_t get_debug_event(debug_event_t *event, int timeout_ms);
  inline drc_t resume(const debug_event_t *event, qstring *errbuf=NULL);
  inline void set_exception_info(const exception_info_t *info, int qty);
  inline void suspended(bool dlls_added, thread_name_vec_t *thr_names=NULL);
  inline drc_t thread_suspend(thid_t tid, qstring *errbuf=NULL);
  inline drc_t thread_continue(thid_t tid, qstring *errbuf=NULL);
  inline drc_t set_resume_mode(thid_t tid, resume_mode_t resmod, qstring *errbuf=NULL);
  inline drc_t read_registers(thid_t tid, int clsmask, regval_t *values, qstring *errbuf=NULL);
  inline drc_t write_register(thid_t tid, int regidx, const regval_t *value, qstring *errbuf=NULL);
  inline drc_t thread_get_sreg_base(ea_t *answer, thid_t tid, int sreg_value, qstring *errbuf=NULL);
  inline drc_t get_memory_info(meminfo_vec_t &ranges, qstring *errbuf=NULL);
  inline drc_t read_memory(size_t *nbytes, ea_t ea, void *buffer, size_t size, qstring *errbuf=NULL);
  inline drc_t write_memory(size_t *nbytes, ea_t ea, const void *buffer, size_t size, qstring *errbuf=NULL);
  inline drc_t check_bpt(int *bptvc, bpttype_t type, ea_t ea, int len);
  inline drc_t update_bpts(int *nbpts, update_bpt_info_t *bpts, int nadd, int ndel, qstring *errbuf=NULL);
  inline drc_t update_lowcnds(int *nupdated, const lowcnd_t *lowcnds, int nlowcnds, qstring *errbuf=NULL);
  inline int open_file(const char *file, uint64 *fsize, bool readonly, qstring *errbuf=NULL);
  inline void close_file(int fn);
  inline ssize_t read_file(int fn, qoff64_t off, void *buf, size_t size, qstring *errbuf=NULL);
  inline ssize_t write_file(int fn, qoff64_t off, const void *buf, size_t size, qstring *errbuf=NULL);
  inline ea_t map_address(ea_t off, const regval_t *regs, int regnum);
  inline const void *get_debmod_extensions(void);
  inline bool update_call_stack(thid_t tid, call_stack_t *trace);
  inline ea_t appcall(
          ea_t func_ea,
          thid_t tid,
  ...
  inline drc_t eval_lowcnd(thid_t tid, ea_t ea, qstring *errbuf=NULL);
  inline drc_t send_ioctl(int fn, const void *buf, size_t size, void **poutbuf, ssize_t *poutsize, qstring *errbuf=NULL);
...
inline bool debugger_t::update_call_stack(thid_t tid, call_stack_t *trace)
{
  return notify_drc(ev_update_call_stack, tid, trace) != DRC_NONE;
}
