...
#define CTXF_STACK        0x00008 // stack view
...
struct outctx_t : public outctx_base_t
{
  ...
  virtual void idaapi out_mnem(int width=8, const char *postfix=NULL) newapi;
  ...
  virtual void idaapi out_custom_mnem(
        const char *mnem,
        int width=8,
        const char *postfix=NULL) newapi;
...
#define GH_PRINT_ALL_BUT_BYTESEX (GH_PRINT_PROC_AND_ASM | GH_PRINT_HEADER)
  virtual void idaapi gen_header(
        ...
        const char *proc_flavour = NULL) newapi;
};
...
///
/// \param out    output buffer (can be NULL)
