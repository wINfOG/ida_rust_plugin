...
#define CF_HLL  0x10000   ///< Instruction may be present in a high level
                          ///< language function.
#define CF_CHG7 0x020000  ///< The instruction modifies the 7th operand
#define CF_CHG8 0x040000  ///< The instruction modifies the 8th operand
#define CF_USE7 0x080000  ///< The instruction uses value of the 7th operand
#define CF_USE8 0x100000  ///< The instruction uses value of the 8th operand
//@}
};
...
struct event_listener_t
{
  ...
  DEFINE_VIRTUAL_DTOR(event_listener_t)
};
...
#define HKCB_AUTODEL 0x0002 ///< automatically delete event listener object after uninstalling it
//@}
...
/// \return success
/// Uninstalling a listener is necessary only for global listeners and
/// listeners with undefined owner. Other listeners are uninstalled automatically
/// when the owner module is unloaded. The uninstalled listener will be
/// automatically deleted by this call if it was installed with the HKCB_AUTODEL
// flag
...
#define REG_SPOIL 0x80000000L
  ...
  enum event_t
  {
    ...
    ev_is_insn_table_jump,      ///< Determine if instruction is a table jump or call.
                                ///< If #CF_JUMP bit cannot describe all kinds of table
                                ///< jumps, please define this callback.
    ...
    ev_replaying_undo,          ///< Replaying an undo/redo buffer
                                ///< \param action_name (const char *) action that we perform undo/redo for. may be NULL for intermediary buffers.
    ...
    ev_ending_undo,             ///< Ended undoing/redoing an action
                                ///< \param action_name (const char *) action that we finished undoing/redoing. is not NULL.
                                ///< \param is_undo (bool) true if performing undo, false if performing redo
    ...
    ev_calc_varglocs,           ///< Calculate locations of the arguments that correspond to '...'.
                                ...
                                ///< \param[out] aux_regs    (::regobjs_t *) buffer for hidden register arguments, may be NULL
                                ///< \param[out] aux_stkargs (::relobj_t *) buffer for hidden stack arguments, may be NULL
  ...
  inline static ssize_t is_insn_table_jump(const insn_t &insn);
  inline static ssize_t loader_elf_machine(linput_t *li, int machine_type, const char **p_procname, proc_def_t **p_pd);
  ...
  inline static ssize_t treat_hindering_item(ea_t hindering_item_ea, flags_t new_item_flags, ea_t new_item_ea, asize_t new_item_length);
  ...
  inline static ssize_t realcvt(void *m, unsigned short *e, unsigned short swt);
  ...
  inline static ssize_t shadow_args_size(int *shadow_size, func_t *pfn);
...
//@}
  ...
  inline ssize_t get_stkarg_offset() const;
...
inline ssize_t processor_t::is_insn_table_jump(const insn_t &insn)
{
  return notify(ev_is_insn_table_jump, &insn);
}
inline ssize_t processor_t::loader_elf_machine(linput_t *li, int machine_type, const char **p_procname, proc_def_t **p_pd)
{
...
inline ssize_t processor_t::treat_hindering_item(ea_t hindering_item_ea, flags_t new_item_flags, ea_t new_item_ea, asize_t new_item_length)
{
...
inline ssize_t processor_t::realcvt(void *m, unsigned short *e, unsigned short swt)
{
...
inline ssize_t processor_t::get_stkarg_offset() const
{
...
inline ssize_t processor_t::shadow_args_size(int *shadow_size, func_t *pfn)
{
...
/// \return main register name (NULL no such register)
...
{
  return is_canon_insn(_ph) ? _ph.instruc[itype-_ph.instruc_start].name : NULL;
}
...
namespace idb_event
{
  ...
  enum event_code_t
  {
    ...
    bookmark_changed,       ///< Boomarked position changed.
                            ...
                            ///< if desc==NULL, then the bookmark was deleted.
...
inline int processor_t::get_proc_index(void)
{
  ...
  int i = 0;
  while ( psnames[i] != nullptr )
  {
    if ( curproc == psnames[i] )
      return i;
    i++;
  }
